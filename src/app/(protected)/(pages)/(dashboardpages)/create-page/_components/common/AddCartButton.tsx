...
const AddCartButton = ({onAddCard}: Props) => {
  ...
  return (
    <motion.div className='w-full relative overflow-hidden' initial={{ height : '0.5rem'}} animate={{ height: showGap ? '2rem' : '0.5rem',transition : {duration : 0.3 , ease : 'easeInOut'}}}
    onHoverStart={()=> setShowGap(true)}
    onHoverEnd={()=> setShowGap(false)}>
        <AnimatePresence>{showGap && 
        <motion.div
